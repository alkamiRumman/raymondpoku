	...
	<div class="modal-dialog modal-xl">
		<div class="modal-content">
			<div class="modal-header">
				<h4 class="modal-title"><b>Update Trade</b></h4>
				<button type="button" class="btn btn-danger float-end close">Close
				</button>
			</div>
			<form class="forms-sample" action="<?= admin_url('update/') . $data->id ?>" method="post"
				  enctype="multipart/form-data">
				<div class="modal-body">
					<div class="row">
						<div class="col-md-2 mb-3">
							<label for="entryDate" class="form-label">Date <sup
										class="text-danger">*</sup></label>
							<div class="input-group flatpickr">
								<input type="text" id="entryDate" name="entryDate" class="form-control"
									   value="<?= date('d M Y', strtotime($data->entryDate)) ?>" data-input required>
								<span class="input-group-text input-group-addon" data-toggle>&#128198;</span>
							</div>
						</div>
						<div class="col-md-2 mb-3">
							<label for="ticker" class="form-label">Ticker </label>
							<input type="text" class="form-control" id="ticker" value="<?= $data->ticker ?>"
								   name="ticker">
						</div>
						<div class="col-md-2 mb-3">
							<label for="side" class="form-label">Trade Side </label>
							<select class="form-select" id="side" name="side">
								<option <?= $data->side == 'Call' ? 'selected' : '' ?> value="Call">Call</option>
								<option <?= $data->side == 'Put' ? 'selected' : '' ?> value="Put">Put</option>
							</select>
						</div>
						<div class="col-md-2 mb-3">
							...
							<input type="number" step="any" class="form-control numeric-input" id="contracts"
								   name="contracts" value="<?= $data->contracts ?>" required>
						</div>
						<div class="col-md-2 mb-3">
						...
						<div class="col-md-2 mb-3">
							<label for="sold" class="form-label">Exit Contract Price <sup
										class="text-danger">*</sup></label>
							<input type="number" step="any" class="form-control numeric-input" id="sold" name="sold"
								   value="<?= $data->sold ?>" required>
						</div>
						<div class="col-md-2 mb-3">
							...
							<div class="input-group">
								...
								<input type="number" step="any"
									   class="form-control <?= $data->subTotal < 0 ? 'text-danger fw-bold' : '' ?>"
									   id="subTotal"
									   value="<?= abs($data->subTotal) ?>" readonly>
						...
						<div class="col-md-2 mb-3">
							...
							<div class="input-group">
								<span class="input-group-text">$</span>
								<input type="number" step="any"
									   class="form-control <?= $data->amount < 0 ? 'text-danger fw-bold' : '' ?>"
									   id="amount"
									   value="<?= abs($data->amount) ?>" readonly>
								<input type="hidden" id="amount_original" name="amount" value="<?= $data->amount ?>">
							</div>
						</div>
						<div class="col-md-2 mb-3">
							...
							<div class="input-group">
								<span class="input-group-text">$</span>
								<input type="number" step="any"
									   class="form-control <?= $data->total < 0 ? 'text-danger fw-bold' : '' ?>"
									   id="total" readonly
									   value="<?= abs($data->total) ?>">
								<input type="hidden" id="total_original" name="total" value="<?= $data->total ?>">
							</div>
						</div>
						<div class="col-md-2 mb-3">
							...
							<div class="input-group" id="percentage_group">
								<input type="number" step="any"
									   class="form-control <?= $data->percentage < 0 ? 'text-danger fw-bold' : '' ?>"
									   id="percentage"
									   data-input readonly value="<?= abs($data->percentage) ?>">
								<span class="input-group-text input-group-addon" data-toggle>%</span>
						...
						<div class="col-md-6 mb-3">
							...
							<div class="input-group">
								<textarea rows="4" class="form-control" id="note"
										  name="note"
										  placeholder="Enter your note for the trade here"><?= $data->ticker ?></textarea>
							</div>
						...
						<div class="col-md-6">
							...
							<div class="card-body">
								<img width="200" style="height: 180px;"
									 ...
									 src="<?= $data->photo != '' ? base_url('uploads/' .
											 $data->id . '/' . $data->photo) : base_url('assets/images/others/noImage.png') ?>">
							</div>
						</div>
					</div>
				</div>
